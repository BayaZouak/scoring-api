...
def get_prediction_from_api(client_features):
    ...
    try:
        ...
        return response.json()  # La réponse contient score, shap local/global etc.
...
if 'api_result' in st.session_state and st.session_state['api_result']['SK_ID_CURR'] == client_id:
    ...
    with col_jauge:
        ...
        confidence_threshold = (1 - BEST_THRESHOLD) * 100
    ...
    df_details = pd.Series({k: v for k, v in current_data.items() if k not in ['SK_ID_CURR', 'TARGET']}).rename('Valeur Client').to_frame()
    ...
    with tab_explicability:
        ...
        if explanation_type == 'Locale (Client)':
            ...
            data_array = [current_data[f] for f in feature_names]
            e = shap.Explanation(
                np.array(shap_local),
                base_value,
            ...
            shap_global = result['shap_global']
            feature_names = result['feature_names']
            importance_df = pd.DataFrame({
                'Feature': feature_names,
                'Importance': np.abs(shap_global)
