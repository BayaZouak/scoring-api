...
API_EXPLAIN_URL = f"{BASE_API_URL}/explain" # Nouvel URL
BEST_THRESHOLD = 0.52
...
def load_model_and_explainer():
    ...
    # CONSERVÉE pour la compatibilité et l'analyse LOCALE du dashboard
    ...
    try:
        ...
        # Chargement des données de référence (pour le SHAP GLOBAL et le Preprocessor)
        ...
        # Création de l'explainer (utilisé seulement pour le SHAP GLOBAL dans le dashboard)
...
if st.sidebar.button("Calculer le Score (API)", key="calculate_score_quick"):
    ...
    if api_result:
        # Appel 2: Explication SHAP
        shap_result = get_api_result(data_to_send, endpoint="explain")

        if shap_result:
            st.session_state['api_result'] = api_result
            st.session_state['shap_result'] = shap_result # Sauvegarde du résultat SHAP de l'API
...
with st.sidebar.form(key=f"form_{client_id}"):
    ...
    # Conversion d'entrée (simple conversion str/float/int)
    for feature, value in client_data_raw.items():
        if feature not in ['SK_ID_CURR', 'TARGET']:

            # Utiliser st.number_input ou st.text_input pour une meilleure expérience
            if full_population_stats.get(feature, {}).get('type') == 'num':
                ...
                edited_data[feature] = st.number_input(
                    f"{feature}",
                    value=default_value,
                    key=f"edit_num_{feature}_{client_id}"
                )
            else:
                ...
                edited_data[feature] = st.text_input(
                    f"{feature}",
                    value=default_value,
                    key=f"edit_text_{feature}_{client_id}"
                )
...
if 'api_result' in st.session_state and st.session_state['api_result']['SK_ID_CURR'] == client_id:
    ...
    with tab_explicability:
        ...
        if 'shap_result' in st.session_state and explainer and X_ref_processed is not None:

            # Utiliser les noms de features de l'API (qui devraient correspondre à ceux chargés localement)
            api_feature_names = st.session_state['shap_result'].get('feature_names_processed', feature_names_processed)
            ...
            try:
                if explanation_type == 'Locale (Client)':
                    ...
                    # Cette partie utilise le modèle et les données de référence chargés LOCALEMENT
                    @st.cache_data(show_spinner=False)
                    def get_global_shap_values(_explainer, X_ref_processed):
                        # On limite l'échantillon pour la rapidité
                    ...
                    importance_df = pd.DataFrame({
                        'Feature': api_feature_names, # On utilise les noms de l'API/Local pour la cohérence
                        'Importance': shap_sum
