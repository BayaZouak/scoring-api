...
def get_transformed_data(pipeline, df):
    """Transforme les données et retourne DataFrame + noms de features"""
    preprocessor = pipeline[:-1]  # Tout sauf le classifieur final
    ...
    return pd.DataFrame(X_trans, columns=feature_names), feature_names
...
async def shap_local(client_data: ClientFeatures):
    try:
        df = pd.DataFrame([client_data.model_dump()])
        df = clean_column_names(df)
        X_trans_df, feature_names = get_transformed_data(model_pipeline, df)
...
async def shap_global():
    try:
        ...
        X_trans_df, feature_names = get_transformed_data(model_pipeline, df_ref)
        explainer = shap.TreeExplainer(model_pipeline.steps[-1][1])
        shap_vals = explainer.shap_values(X_trans_df)
        ...
        raise HTTPException(status_code=500, detail=f"Erreur SHAP global : {e}")import pandas as pd
import numpy as np
import joblib
from fastapi import FastAPI, HTTPException
from typing import Optional
